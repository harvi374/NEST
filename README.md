...
This repository constitutes the reference implementation accompanying the NEST paper: a federated Anti-Money-Laundering (AML) detection system for the Elliptic Bitcoin transaction graph, designed to address two coupled challenges inherent to cross-institutional financial graph learning:

1. **Hardware heterogeneity.** Participating institutions differ substantially in the prototype dimensionality they can afford to compute and transmit (8, 16, 32, or 64 dimensions). NEST addresses this by learning a single 64-dimensional embedding space in which every lower-dimensional prefix remains a semantically meaningful representation in its own right, following the principle of Matryoshka Representation Learning. This eliminates the information loss associated with conventional zero-padding or truncation.
2. **Chronological non-IID drift.** Institutions observe disjoint temporal windows of transaction activity, producing distributional shift across clients. FedProx-style proximal regularization of the shared encoder, combined with round-wise prototype re-alignment, maintains coordination across clients despite this drift.

The codebase is organized as an installable Python package (`src/`) accompanied by a set of driver notebooks that serve as the primary execution entry points; `src/` functions as the shared library imported throughout.

## Note on Nomenclature

The project has been referred to by several internal names over the course of its development, each of which persists in the code and accompanying documentation as part of the same continuous body of work:

- **FeTA** — the original project framing, retained in the `src/__init__.py` docstring and in the package name `feta-aml` declared in `setup.py`.
- **PGFCL** (Prototype-Guided Federated Contrastive Learning) — the name given to the main training loop, implemented as `run_pgfcl` in `src/training/federated.py`.
- **EP-FedProto** ("Embedding-Prefix FedProto") — the designation used for the tiered, nested-budget variant, implemented via `supervised_round_client_ep` and the `trunc_dim` fixed-tier baseline in `FullGAT.forward`.
- **NEST** — the name under which the final nested-representation architecture, together with Rank-Aware ContribAgg, is presented in the CODS-COMAD '26 submission.

For readers cross-referencing the paper against the implementation: the paper's NEST corresponds to this repository's PGFCL / EP-FedProto machinery, specifically the combination of `rank_aware_contrib_agg()`, `multi_budget_proto_loss()`, and `run_pgfcl()`.
...
**`FullGAT`** (`src/models/full_gat.py`) — composes encoder and head. Its `forward(..., trunc_dim=None)` argument implements the fixed-tier baseline: when set, it zero-masks embedding dimensions `>= trunc_dim` **before** the classification head, directly realizing the prefix operator `phi_d(z) = z[:d]` defined in the paper. This mechanism underlies the EP-FedProto v3 fixed-tier comparison.
...
| `fedavg_state_contrib` | `fedavg.py` | FedAvg using externally supplied weights (from ContribAgg or GradGuard); this is the function invoked by the main training loop |
...
The priority order within `run_pgfcl` should be noted: **GradGuard takes precedence over ContribAgg** when both are enabled (`if cfg.use_grad_guard: ... elif cfg.use_contrib_agg: ...`); the two mechanisms are not currently combined additively.
...
- **`label_aware_edge_supcon`** — the loss employed during SSL pre-warm: illicit nodes serve as anchors, other illicit nodes as positives, and licit nodes as negatives — a supervised-contrastive variant that pre-shapes the embedding space around the minority class prior to classifier training.
...
`case_studies/peeling_chain/` — an in-depth structural study. It is explicitly documented as an illustrative case study rather than an assertion about ground-truth laundering topology (the notebook's own annotation: *"a peeling-chain-like structural illustration, not a claim that the Elliptic labels establish..."*).
...
- Cross-tier case study (peeling-chain, Section 4.6): NEST 64→8 achieves Client-B F1 of 0.5288/0.6958/0.6967 across three seeds, compared to FedProto-8's 0.4919/0.5848/**0.2847**. FedProto-8 collapses on one seed; NEST does not.
- Ablation deltas: removing FedProx produces the largest degradation (ΔF1 = −0.040); removing EMA aggregation, ContribAgg, or focal loss individually produces smaller degradations of roughly −0.003 to −0.009.
...
- Rank-Aware ContribAgg requires a sufficient number of participating clients to populate high-dimensional segments; sparse coverage at d=32/64 weakens those segments.
- Performance gains are most pronounced on **chronologically structured** graphs such as Elliptic. On the larger, less temporally structured IEEE-CIS graph, FedProto-8 remains competitive on F1/AUC, indicating that the tiered design is best suited to federated data with a genuine temporal component.
- `src/data/ieee_cis.py` is currently a stub; IEEE-CIS graph construction is implemented inline within the external-validation notebooks rather than in the shared library.
- Representation budgets are fixed at `{8, 16, 32, 64}` rather than dynamically selected per round.
- GradGuard and ContribAgg are mutually exclusive within the current training loop (GradGuard takes precedence) rather than combined.
